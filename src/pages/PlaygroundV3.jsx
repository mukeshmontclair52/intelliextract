...
function ExtractOutput({ file }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-4 pt-4 pb-3 border-b border-slate-100 flex-shrink-0">
        <p className="text-xs font-semibold text-slate-600">Extraction Schema</p>
        <p className="text-xs text-slate-400 mt-0.5">Choose how to define what to extract.</p>
      </div>
      <div className="flex-1 overflow-auto p-4 space-y-3">
        {/* Config Picker */}
        <div className="rounded-xl border border-slate-200 overflow-hidden bg-white">
          <button onClick={() => setShowPicker(!showPicker)} className="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium text-slate-600">Load from Document Config</span>
            </div>
            <ChevronDown className={cn("w-3.5 h-3.5 text-slate-400 transition-transform", showPicker && "rotate-180")} />
          </button>
          {showPicker && (
            <div className="border-t border-slate-100">
              {MOCK_DOC_CONFIGS.map(cfg => (
                <button key={cfg.id} onClick={() => { setPrompt(cfg.prompt); setMode("ai"); setShowPicker(false); }} className="w-full text-left px-4 py-2.5 text-xs hover:bg-violet-50 transition-colors">
                  <p className="font-medium text-slate-700">{cfg.name}</p>
        ...
        {/* AI Mode */}
        <div className={cn("rounded-xl border overflow-hidden bg-white transition-all", mode === "ai" ? "border-violet-200" : "border-slate-200")}>
          <button onClick={() => setMode(mode === "ai" ? null : "ai")} className="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-violet-500 to-indigo-500 flex items-center justify-center">
                <Wand2 className="w-3 h-3 text-white" />
              </div>
              <span className="text-xs font-medium text-slate-700">Generate with AI</span>
            </div>
            <ChevronDown className={cn("w-3.5 h-3.5 text-slate-400 transition-transform", mode === "ai" && "rotate-180")} />
          </button>
          {mode === "ai" && (
            <div className="px-4 pb-4 border-t border-slate-100 pt-3 space-y-2">
              <textarea
                className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-xs text-slate-700 placeholder:text-slate-300 outline-none focus:ring-2 focus:ring-violet-200 resize-none"
                rows={3}
                placeholder="Describe what to extract (e.g. invoice number, date, total amount)…"
                value={prompt}
        ...
        <button
          ...
          className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 text-xs font-medium transition-all", mode === "json" ? "border-violet-300 bg-violet-50 text-violet-700" : "border-slate-200 bg-white text-slate-500 hover:border-slate-300")}
        >
          <Code2 className="w-4 h-4" /> Upload JSON Schema
        </button>
        <button
          onClick={() => setMode("scratch")}
          className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 text-xs font-medium transition-all", mode === "scratch" ? "border-violet-300 bg-violet-50 text-violet-700" : "border-slate-200 bg-white text-slate-500 hover:border-slate-300")}
        >
